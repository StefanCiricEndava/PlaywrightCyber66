...
test("Alokai e2e test", async ({ page }) => {
  ...
  await expect(pm.onHomePage().searchResults).toContainText(
    dataset.product3.category
  );
  ...
  await expect(
    page.locator(
      `details[data-testid="accordion-item"] div label span input[value="${dataset.product3.brand}"]`
    )
  ).toBeChecked();
  ...
  await pm
    .onHomePage()
    .clickOnSpecificProduct(dataset.product3.name, dataset.product3.sku);
  await page.waitForLoadState("networkidle");

  // Assertation on product page to be sure that we selected right product
  await expect(page.url()).toContain(
    `${dataset.product3.sku}?sku=${dataset.product3.sku}`
  );
  await expect(pm.onProductPage().productName).toHaveText(
    dataset.product3.name
  );
  await expect(pm.onProductPage().productPrice).toHaveText(
    `$${dataset.product3.price.toString()}.00`
  );
  ...
  await expect(pm.onProductPage().notifications).toContainText(
  ...
  await expect(pm.onCartPage().productTitle).toContainText(
    dataset.product3.name
  );
  await expect(pm.onCartPage().productPrice).toHaveText(
    `$${dataset.product3.price.toString()}.00`
  );
  ...
  await expect(pm.onCartPage().productTotal).toHaveText(
    `$${(dataset.product3.price * 2).toString()}.00`
  );
